...
data class CalendarUiState(
    ...
    val selectedDatesFormattedWithYear: String
        get() {
            if (selectedStartDate == null) return ""
            var output = selectedStartDate!!.format(LONG_DATE_FORMAT)
            if (selectedEndDate != null) {
                output += " - ${selectedEndDate.format(LONG_DATE_FORMAT)}"
            }
            return output
        }
    ...
    fun setNullDates(): CalendarUiState {
        return copy(selectedStartDate = null, selectedEndDate = null)

    }

    companion object {
        private val SHORT_DATE_FORMAT = DateTimeFormatter.ofPattern("MMM dd")
        private val LONG_DATE_FORMAT = DateTimeFormatter.ofPattern("MMM dd YYYY")
    }
